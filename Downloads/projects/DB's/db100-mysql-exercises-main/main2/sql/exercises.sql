...
# total_rental_cost     rental_duration * rental_rate
    ...
    SELECT 
        title, description, rental_duration, rental_rate, 
...
select staff_id, first_name, last_name, address_id, picture, email, store_id,
    active, username, last_update from staff where password is not null;

# 3h. Select all columns minus the password column from the staff table for rows that do not contain a password.
select staff_id, first_name, last_name, address_id, picture, email, store_id,
    active, username, last_update from staff where password is null;
...
# total_rental_cost     rental_duration * rental_rate -- come back to this one
...
select title, length, description, rental_rate from film where description REGEXP('\\bDatabase\\b') and length > 180;
# ---------------------------------------------------------#
...
select payment_id, payment_date, amount from payment where amount > 5 order by payment_id limit 50 offset 51;
...
select title, description, special_features, length, rental_duration from film limit 10;
...
select customer.last_name as customer_last_name, customer.first_name as customer_first_name, actor.last_name as actor_last_name, actor.first_name as actor_first_name from customer right join actor on customer.last_name = actor.last_name; 
...
select customer.last_name as customer_last_name, customer.first_name as customer_first_name, actor.last_name as actor_last_name, actor.first_name as actor_first_name from customer inner join actor on customer.last_name = actor.last_name; 
...
select first_name, last_name, 
    address.address as address, address.address2 as address2, address.district as district, 
        address.postal_code as postal_code, city.city as city from staff 
